...
def realtime():
    ...
    while rval:
        ...
        update(histarray2)
        val1=(classes[list(y_pred[0]).index(y_pred[0].max())])
        cv2.putText(frame,classes[list(y_pred[0]).index(y_pred[0].max())], (50,50), cv2.FONT_HERSHEY_PLAIN , 1, 255)
        print(val1)

        #cv2.putText(frame,"Hello", (50,150), cv2.FONT_HERSHEY_PLAIN , 1, 255)
